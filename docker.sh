...
SETUP_DB_FILE="Docker/setupDB.sh"
...
sed -i -e "s/USER/${1}/g" ${WORKSPACE}/src/*/resources/hikari.properties 
sed -i -e "s/PASSWORD/${2}/g" ${WORKSPACE}/src/*/resources/hikari.properties 
sed -i -e "s/PASSWORD/${2}/g" ${WORKSPACE}/Docker/setupDB.sh
...
docker cp ${WORKSPACE}/src/main/resources/DB/ ${DOCKER_CONTAINER_DB}:/DB
docker cp ${WORKSPACE}/src/test/resources/DB/ ${DOCKER_CONTAINER_DB}:/DB-test
docker cp ${WORKSPACE}/${SETUP_DB_FILE} ${DOCKER_CONTAINER_DB}:/setupDB.sh
