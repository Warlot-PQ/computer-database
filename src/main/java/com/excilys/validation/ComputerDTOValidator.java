...
public class ComputerDTOValidator {
	private final int MAX_NAME_SIZE = 255, MAX_INTRODUCED_SIZE = 10, MAX_DISCONTINUED_SIZE = 10, MAX_COMPANY_SIZE = 20;	
	...
	private void checkIntroducedDate() {
		...
		if (computerDTOToValidate.getIntroduced() != null && computerDTOToValidate.getIntroduced().matches(DATE_PATTERN_MATCHING) == false) {
	...
	private void checkDiscontinuedDate() {
		...
		if (computerDTOToValidate.getDiscontinued() != null && computerDTOToValidate.getDiscontinued().matches(DATE_PATTERN_MATCHING) == false) {
