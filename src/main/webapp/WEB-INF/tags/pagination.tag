...
<ul class="pagination">
	<c:if test="${current ne start}">
		...
		<li><myLib:link classes="btn btn-default" page="${start}" limit="${limit}" att="aria-label=\"Next\"">
			First
		</myLib:link></li>
	</c:if>
	...
	<c:if test="${currentPage ne end}">
		...
		<li><myLib:link classes="btn btn-default" page="${end}" limit="${limit}" att="aria-label=\"Next\"">
			Last
		</myLib:link></li>
	</c:if>
</ul>